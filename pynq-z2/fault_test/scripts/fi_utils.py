...
def bitflip_injection(golden_bitstream, injection_num):
    ...
    # ALE: PYNQ-Z2 - ZYNQ XC7Z020 Artix-7 FPGA
    # syn_word: 0xAA995566
    # Total Frames: Calculated dynamically
    # Words per frame: 101
    # Bytes per word: 4
    ...
    start_byte, end_byte, total_frames = bitstream_analyzer(golden_bitstream)
...
# Ale: Updated bitstream_analyzer to dynamically calc. and return total_frames
def bitstream_analyzer(bitstream):
    with open(bitstream, "rb") as f:
        bitstream_content = f.read()

    # 7-Series Sync Word: AA 99 55 66
    start_pattern_match = re.search(b'\xAA\x99\x55\x66', bitstream_content) 

    if start_pattern_match == None:
        exit("Error: 7-Series Sync Word not found!")
    
    # The frames start shortly after the sync word
    start_byte = start_pattern_match.start() + 4
    
    # The end of the payload is essentially the end of the file (minus a small footer)
    end_byte = len(bitstream_content) - 100 # Safe buffer for the footer
    
    # Dynamically calculate total frames
    bytes_per_frame = 101 * 4
    total_frames = (end_byte - start_byte) // bytes_per_frame
    
    print(f"Detected PYNQ-Z2 Bitstream! Total Frames: {total_frames}")
    
    return start_byte, end_byte, total_frames
