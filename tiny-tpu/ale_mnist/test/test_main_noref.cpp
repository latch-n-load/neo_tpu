...
// static const uint64_t MAX_SIM_TIME = 10000000000; // 10ms max (1M cycles)
...
int main(int argc, char** argv) {
    ...
    // Apply reset
    apply_reset(10); // Release reset after 10 cycles (11th rising edge)
    
    // Pulse start signal
    std::cout << "[TB] Asserting start pulse..." << std::endl;
    toggle_clock(); // Advance time by 1/2 CC
    top->start = 1; 
    toggle_clock(); 
    toggle_clock(); // Advance time by 1 CC
    top->start = 0; 
    toggle_clock(); 
    std::cout << "[TB] Cycle " << cycle_count << ": Start pulse complete" << std::endl;
    ...
    while (sim_time < MAX_SIM_TIME) {
        ...
        if (top->clk) {
            ...
            if (top->debug_state == 3 && top->debug_current_layer == 0) { // STATE_LOAD_INPUT TODO: How would neorv32 know TPU state?
                uint16_t addr = top->pixel_addr_out;
                if (addr < PIXELS) {
                    top->pixel_data_in = test_image[addr];
                    // Log pixel data being fed into DUT
                    if (addr == 0 || addr < 5 || addr % 100 == 0 || test_image[addr] != 0) { // Log first few pixels, every 100th pixel, and any non-zero pixels
